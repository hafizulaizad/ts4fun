...
fs_1.default.readdir(directoryPath, function (err, files) {
    ...
    files.forEach(function (file) {
        ...
        const targetString = file;
        const rExp = /^[a-zA-Z]{4}[0-9]{4}_[0-9]{12}_section.csv/gi;
...
function readFile(targetString) {
    let result = [];
    //Read file
    const data = fs_1.default.readFileSync(targetString);
    //Split rows
...
function checkNameWData(inputArr, filenameArr) {
    ...
    for (let i = 0; i < inputArr.length; i++) {
        if (filenameArr[0] === inputArr[i]["PartNo"] &&
